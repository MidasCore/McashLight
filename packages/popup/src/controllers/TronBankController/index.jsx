...
import { BANK_STATE, APP_STATE } from '@mcashlight/lib/constants';
import { NavBar, Button, Modal, Toast } from 'antd-mobile';
import Utils from '@mcashlight/lib/utils';
import { getBankDefaultDataApi, getBankIsRentApi, getBankBalanceEnoughApi, postBankOrderApi } from '@mcashlight/popup/src/fetch/tronLending/tronLending';
...
class BankController extends React.Component {
    ...
    async defaultDataFun() {
        ...
        this.setState({
            rentNumMin: defaultData.rental_amount_min / Math.pow(10, 8),
            rentNumMax: defaultData.rental_amount_max / Math.pow(10, 8),
            ...
            defaultUnit: {
                ...
                cost: defaultData.pay_amount / Math.pow(10, 8),
        ...
        if(!Utils.validatInteger(rentVal)) {
            ...
            if(rentVal <= rentNumMax && rentVal >= rentNumMin) {
                if(_type === 2) {
                    ...
                    const selectedaAddress = selected.address;
                    if(curaAddress === '') address = selectedaAddress; else address = curaAddress;
                    const isValid = await PopupAPI.isValidOverTotal(address, rentVal * Math.pow(10, 8), requestUrl);
    ...
    handlerInfoConfirm() {
        ...
        const { selected } = this.props.accounts;
        const currentBalance = selected.balance / Math.pow(10, 8);
    ...
    rentDealSendFun(e) {
        ...
        const freezeAmount = rentNum.value * Math.pow(10, 8);
    ...
    render() {
        ...
        const orderList = [
            ...
            { id: 'BANK.RENTINFO.RENTNUM', tip: 1, value: `${rentNum.value}MCASH` },
            { id: 'BANK.RENTINFO.RENTDAY', type: 3, value: rentDay.value },
            { id: 'BANK.RENTINFO.PAYNUM', type: 0, value: `${rentUnit.cost}MCASH` },
        ];
                        ...
                        <div className='accountContent'>
                            <section className='accountInfo infoSec'>
                                ...
                                <div className='balance'>
                                    <FormattedMessage id='BANK.INDEX.BALANCE' values={{ amount: selected.balance / Math.pow(10, 8) }}/>
                                </div>
                            </section>
                        ...
                        <div className='rentContent'>
                            ...
                            {rentNum.valid && rentDay.valid ?
                                <section className='calculation'>
                                    <div className='info'>
                                        <span>{rentNum.value}MCASH*{rentDay.value}</span>
                                        ...
                                        <span className='pointColor'>
                                            <FormattedMessage id='BANK.INDEX.RENTCONST' /> {rentUnit.cost} MCASH
                                        </span>
                                    </div>
                                    <div className='curNum'>
                                        {
                                            language === 'en' ?
                                                <span>
                                                    (<span className='pointColor'>{ saveCost.toFixed(2) }MCASH </span>saved than burn-MCASH,<span className='pointColor'>{rentNum.value}MCASH</span> required to freeze)
                                                </span>
                                                :
                                                <span>
                                                    (
                                                    ...
                                                    <span className='pointColor'>{rentNum.value}MCASH</span>
                                                    )
                                                </span>
                                        }
                                    </div>
                                    ...
                                    <div className='info'>
                                        {
                                            language === 'en' ?
                                                <span>
                                                    renting {defaultUnit.num * 10}k energy * {defaultUnit.day} day costs {defaultUnit.cost}MCASH
                                                </span>
                                                :
