        ...
        return(
            <div>
                {recordList.map((val, key) => {
                    ...
                    const payMonkey = val.pay_amount / Math.pow(10, 8);
                    return(
                        <div key={ key } className='recordList' onClick={ () => { this.toMoreDetail(val.id); } }>
                            ...
                            <div className='recordCont'>
                                <section className='recordLeftInfo'>
                                    <div><FormattedMessage id='BANK.RENTRECORD.RENTDETAIL'/>{val.freeze_amount / Math.pow(10, 8)}MCASH*{val.days}<FormattedMessage id='BANK.RENTRECORD.TIMEUNIT'/></div>
